                                                             ...
                                                             try{
                                                                 sh( label: 'Running Tox',
                                                                     script: """python3 -m venv venv && venv/bin/pip install uv
                                                                                ...
                                                                                uvx -p ${version} --with requirements-dev.txt --with tox-uv tox run -e ${toxEnv}
                                                                             """
                                                                     )
                                                             } catch(e) {
                                     ...
                                     node('docker && windows'){
                                         docker.image('python').inside('--mount source=python-tmp-pyhathiprep,target=C:\\Users\\ContainerUser\\Documents'){
                                             try{
                                                 ...
                                                 envs = bat(
                                                     label: 'Get tox environments',
                                                     script: '@.\\venv\\Scripts\\uvx --quiet --with requirements-dev.txt --with tox-uv tox list -d --no-desc',
                                                     returnStdout: true,
                                                 ).trim().split('\r\n')
                                             } finally{
                                                 cleanWs(
                                            ...
                                            if(isUnix()){
                                                sh(
                                                    ...
                                                    script: '''python3 -m venv venv
                                                               ...
                                                               uvx --with requirements-dev.txt --with tox-uv tox
                                                            '''
                                                )
                                            } else {
                                                bat(
